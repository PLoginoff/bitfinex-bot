...
class Bitfinex
{
    ...
    private function send_auth_endpoint_request($data, $post = '{}')
    {
        $ch = curl_init();
    ...
    {
        $request = $this->build_url_path("auth/w/order/submit");
        $data = [
            "request" => $request
        ];
        $body = [
        ...
        $balances = $this->send_auth_endpoint_request($data, \json_encode($body));
